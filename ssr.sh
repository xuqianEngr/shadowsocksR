...
LotServer_installation_status(){
	[[ ! -e ${LotServer_file} ]] && echo -e "${Error} 没有安装 LotServe，请检查 !" && exit 1
}
BBR_installation_status_debian(){
	if [[ ! -e ${BBR_file} ]]; then
		echo -e "${Error} 没有发现 BBR脚本，开始下载..."
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/shadowsocksR/master/bbr/bbr_debian.sh; then
			echo -e "${Error} BBR 脚本下载失败 !" && exit 1
		else
			echo -e "${Info} BBR 脚本下载完成 !"
			chmod +x bbr_debian.sh
		fi
	fi
}
BBR_installation_status_centos(){
	if [[ ! -e ${BBR_file} ]]; then	
		echo -e "${Error} 没有发现 BBR脚本，开始下载..."
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/shadowsocksR/master/bbr/bbr_centos.sh; then
			echo -e "${Error} BBR 脚本下载失败 !" && exit 1
		else
			echo -e "${Info} BBR 脚本下载完成 !"
			chmod +x bbr_centos.sh
		fi
	fi
}
BBR_installation_status(){
	if [[ ${release} = "centos" ]]; then
		BBR_installation_status_centos
		BBR_file="${file}/bbr_centos.sh"
	 else
		BBR_installation_status_debian
		BBR_file="${file}/bbr_debian.sh"
	fi
}
# 设置 防火墙规则
Add_iptables(){
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/xuqianEngr/shadowsocksR/raw/master/ssr/manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/ssr/master/ssr_for_linux/ssr/config/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/ssr/master/ssr_for_linux/ssr/config/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/shadowsocksR/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xuqianEngr/shadowsocksR/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	...
	wget -N --no-check-certificate "https://raw.githubusercontent.com/xuqianEngr/shadowsocksR/master/ssr.sh" && chmod +x ssr.sh
...
echo -e "  ShadowsocksR 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ---- just for test and study version ----
  	------Improve by Warren xu-------
